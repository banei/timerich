  ...
  return (
    <div className={`budget-recon ${r.aligned ? "budget-recon-ok" : "budget-recon-diff"}`}>
      <p className="budget-recon-lead">
        {r.aligned ? (
          <>
            计划合计与月预算<strong>一致</strong>（{fmt(r.total_planned)}）。
          ...
          <>
            ...
            <strong className={r.delta > 0 ? "text-up" : "text-down"}>
              {r.delta > 0 ? "多" : "少"} {fmt(Math.abs(r.delta))}
            </strong>
            （预算 {fmt(r.budget)} → 计划 {fmt(r.total_planned)}）。
          </>
        )}
      </p>
        ...
        <ol className="budget-recon-steps">
          {r.steps.map((step) => (
            <li key={step.key}>
              ...
              <span className="font-num">{fmt(step.amount)}</span>
          ...
          <div className="budget-recon-overrides">
            ...
            <ul>
              {r.override_adjustments.map((a) => (
                <li key={a.bucket}>
                  {a.label}：{fmt(a.from_amount)} → {fmt(a.to_amount)}（{fmtDelta(a.delta)}）
                </li>
              ))}
            </ul>
          </div>
